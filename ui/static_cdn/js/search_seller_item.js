...
searchSellerItemApp.controller('searchSellerItemCtrl', function($scope,$http,$timeout,$window) {
	...
	$scope.is_flag = function()
	{
		...
		var request = $http({
            method: "post",
            url: "http://34.232.210.211:3001/flag-seller-ebay-items/",
            data: form_data,
	...
	$scope.update_all = function()
	{
		var request = $http({
	                    method: "post",
	                    url: "http://34.232.210.211:3001/update-all-ebay-items/",
	                });	
	...
	$scope.refresh_ignored_list = function(page_number)
	{
		...
		$http.get("http://34.232.210.211:3001/get-ebay-sellers-itemm/?format=json&page="+page_number+"&keyword="+$scope.keyword+"&page_by="+$scope.selected_pagination_option+"&order_by="+$scope.selected_sorting_option+"&status=ignored")
		    .then(function(response) {
		    	$scope.ignored_items = [];
		...
		$scope.refresh_unmonitored_list = function(page_number)
	{
		...
		$http.get("http://34.232.210.211:3001/get-ebay-sellers-itemm/?format=json&page="+page_number+"&keyword="+$scope.keyword+"&order_by="+$scope.selected_sorting_option+"&page_by="+$scope.selected_pagination_option+"&status=unmonitored")
		    .then(function(response) {
		    	$scope.unmonitored_items = [];
	...
	{
		 $http.get("http://34.232.210.211:3001/get-ebay-sellers-itemm/?format=json&page="+page_number+"&keyword="+$scope.keyword+"&order_by="+$scope.selected_sorting_option+"&page_by="+$scope.selected_pagination_option+"&status=monitored")
		    .then(function(response) {
		    	$scope.items = [];
