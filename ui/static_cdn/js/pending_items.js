...
pendingItemsApp.controller('pendingItemsCtrl', function($scope,$http,$timeout) {
	...
	$scope.delete = function()
	{
		...
		var request = $http({
                method: "post",
                url: "http://34.232.210.211:3001/delete-pending-ebay-items/",
                data: submit_items,
            }).then(function(response) {
	...
	$scope.refresh_seller_list = function(page_number)
	{
		...
		$http.get("http://34.232.210.211:3001/get-pending-items/?format=json&page="+page_number+"&keyword="+$scope.keyword+"&order_by="+$scope.selected_sorting_option)
		    .then(function(response) {
